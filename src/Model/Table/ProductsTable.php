...
class ProductsTable extends Table
{
    ...
    public function initialize(array $config): void
    {
        ...
        $this->setDisplayField('id_produk');
        $this->setPrimaryKey('id_produk');
    }
    ...
    public function validationDefault(Validator $validator): Validator
    {
        $validator
            ->scalar('nama_produk')
            ->maxLength('nama_produk', 255)
            ->requirePresence('nama_produk', 'create')
            ->notEmptyString('nama_produk');

        $validator
            ->integer('harga')
            ->allowEmptyString('harga');

        $validator
            ->scalar('kategori')
            ->maxLength('kategori', 255)
            ->allowEmptyString('kategori');
